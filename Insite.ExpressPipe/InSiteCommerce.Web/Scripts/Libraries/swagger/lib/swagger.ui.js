...
SwaggerUi = (function(_super) {
  ...
  SwaggerUi.prototype.load = function() {
    ...
    this.options.authorizations = {}; // this blocks the swagger call from using the access_token which would break the call

    // add the access token so try it out calls will work
    var viewingAdminApi = url.indexOf('docs/admin') > 0;
    var token = viewingAdminApi ? window.localStorage['admin-accessToken'] : window.localStorage['accessToken'];
    if (token) {
        var apiKeyAuth = new ApiKeyAuthorization("access_token", token, "query");
        var e = (typeof window !== 'undefined' ? window : exports);
        e.authorizations.add("access_token", apiKeyAuth);
        this.authenticationMessage = "";
    } else {
        this.authenticationMessage = viewingAdminApi ?
            "You must log into the console in order to use the 'Try it out' feature." :
            "You must log into the storefront in order to fully use the 'Try it out' feature.";
    }     
  ...
  SwaggerUi.prototype.render = function() {
    ...
    this.showMessage(this.authenticationMessage);
