<%@ Control Language="C#" EnableViewState="false" AutoEventWireup="false" Inherits="Insite.Admin.CustomCKFinder.ConfigFile" %>
<%@ Import Namespace="System" %>
<%@ Import Namespace="CKFinder.Settings" %>
<%@ Import Namespace="Insite.Admin.CustomCKFinder" %>
<%@ Import Namespace="Insite.Common.Dependencies" %>
<%@ Import Namespace="Insite.Data.Repositories.Interfaces" %>
...
<script runat="server">
    ...
    {
        return DependencyLocator.Current.GetInstance<ICustomCkFinderAuthenticationService>().IsAuthenticated();
    }

    public string[] GetAdditionalExtensions()
    {
        var unitOfWorkFactory = DependencyLocator.Current.GetInstance<IUnitOfWorkFactory>();
        var appSettingsRepository = unitOfWorkFactory.GetUnitOfWork().GetTypedRepository<IApplicationSettingRepository>();
        var additionalExtensionsSetting = appSettingsRepository.GetOrCreateByName<string>("CkFinderAdditionalExtensions");
        
        if (!additionalExtensionsSetting.IsBlank())
        {
            return additionalExtensionsSetting.Trim().Split(',').Select(x => x.Trim()).ToArray();
        }
    ...
    {        
        ...
        Insite.Admin.CustomCKFinder.ResourceType type = ResourceType.Add( "UserFiles" );
        type.Folder = "";
        type.MaxSize = 0;
        ...
        type.AllowedExtensions = type.AllowedExtensions.Concat(GetAdditionalExtensions()).ToArray();
        ...
        type.Folder = "";
        type.MaxSize = 0;
        type.AllowedExtensions = new string[] { "doc", "txt" };
        type.DeniedExtensions = new string[] { };
