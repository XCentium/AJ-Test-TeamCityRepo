...
--
-- Name:	PopulateInsiteOrderHistory
...
-- Altered:
-- NOTE:  ETL OrderHistory must have been populated before running this
-- Test With: EXEC sp_PopulateInsiteOrderHistory 'EXP', 'ServiceUser'
(
...
BEGIN
	...
	BEGIN TRY
		BEGIN TRANSACTION
			...
			;MERGE [Insite.ExpressPipe]..OrderHistory AS Target
			USING
			(
			...
			(
				...
				OR Target.ConversionRate <> Source.ConversionRate
			) THEN
			UPDATE SET 
				Target.ERPOrderNumber = Source.ERPOrderNumber,
			...
			;MERGE [Insite.ExpressPipe]..OrderHistoryLine AS Target
			USING
			(
			...
			(
				...
				OR Target.RequiredDate <> Source.RequiredDate
				OR Target.LastShipDate <> Source.LastShipDate
				OR Target.CustomerNumber <> Source.CustomerNumber
				OR Target.CustomerSequence <> Source.CustomerSequence
